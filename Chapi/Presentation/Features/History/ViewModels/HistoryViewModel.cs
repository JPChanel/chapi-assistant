...
public class HistoryViewModel : ViewModelBase
{
        ...
        Domain.Interfaces.IGitRepository gitRepository)
    {
        ...
        LoadMoreCommand = new AsyncRelayCommand(LoadMoreHistoryAsync);
        ResetSoftCommand = new AsyncRelayCommand<CommitItemViewModel?>(ResetSoftAsync);
        CreateBranchCommand = new AsyncRelayCommand<object?>(CreateBranchFromParameterAsync);
        CreateTagCommand = new AsyncRelayCommand<object?>(CreateTagFromParameterAsync);
    }
    ...
    public AsyncRelayCommand<CommitItemViewModel?> ResetSoftCommand { get; }
    public AsyncRelayCommand<object?> CreateBranchCommand { get; }
    public AsyncRelayCommand<object?> CreateTagCommand { get; }

    private Task CreateBranchFromParameterAsync(object? parameter)
    {
        return parameter switch
        {
            string hash => CreateBranchAsync(hash),
            CommitItemViewModel commit => CreateBranchAsync(commit.Hash),
            _ => Task.CompletedTask
        };
    }

    private Task CreateTagFromParameterAsync(object? parameter)
    {
        return parameter switch
        {
            string hash => CreateTagAsync(hash),
            CommitItemViewModel commit => CreateTagAsync(commit.Hash),
            _ => Task.CompletedTask
        };
    }
