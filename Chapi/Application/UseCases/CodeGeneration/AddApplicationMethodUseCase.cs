...
public class AddApplicationMethodUseCase
{
        ...
        RollbackManager.RollbackEntry? rollbackEntry = null)
    {
        try
        {
            ...
            var result = await AddApplicationMethod.AddMethodAsync(
                projectPath,
                moduleName,
                methodName,
                methodType,
            ...
            if (string.IsNullOrWhiteSpace(result))
                return Result<string>.Fail("No se pudo agregar el método a Application");

            return Result<string>.Success(result);
        }
        catch (Exception ex)
        {
            return Result<string>.Fail($"Error agregando método a Application: {ex.Message}");
        }
    }
}
