...
public class AddApplicationMethodUseCase
{
        ...
        bool useGenericRepository = false)
    {
        try
        {
            ...
            var result = AddApplicationMethod.Add(
                projectPath,
                moduleName,
                operation,
                methodName,
            ...
            if (result == null)
                return Result<RollbackManager.RollbackEntry>.Fail("No se pudo agregar el método a Application");

            return Result<RollbackManager.RollbackEntry>.Success(result);
        }
        catch (Exception ex)
        {
            return Result<RollbackManager.RollbackEntry>.Fail($"Error agregando método a Application: {ex.Message}");
        }
    }
}
