...
public class AddDomainMethodUseCase
{
        ...
        RollbackManager.RollbackEntry? rollbackEntry = null)
    {
        try
        {
            ...
            var result = await AddDomainMethod.Add(
                projectPath,
                moduleName,
                operation,
                methodName,
            ...
            if (result == null)
                return Result<RollbackManager.RollbackEntry>.Fail("No se pudo agregar el método al dominio");

            return Result<RollbackManager.RollbackEntry>.Success(result);
        }
        catch (Exception ex)
        {
            return Result<RollbackManager.RollbackEntry>.Fail($"Error agregando método a Domain: {ex.Message}");
        }
    }
}
