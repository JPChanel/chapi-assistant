...
public class AddApiControllerUseCase
{
        ...
        RollbackManager.RollbackEntry? rollbackEntry = null)
    {
        try
        {
            ...
            var result = await AddApiControllerMethod.AddControllerAsync(
                projectPath,
                moduleName,
                methodName,
                rollbackEntry);
            ...
            if (string.IsNullOrWhiteSpace(result))
                return Result<string>.Fail("No se pudo agregar el controlador API");

            return Result<string>.Success(result);
        }
        catch (Exception ex)
        {
            return Result<string>.Fail($"Error agregando API Controller: {ex.Message}");
        }
    }
}
